...
router.all("/export", async (req, res) => {
  ...
  try {
    ...
    if (format === "pdf") {
      ...
      // ================= LUCKNOW PIE CHARTS =================
      if (table === "sarus_lucknow_population" && chartImage) {

        const chartBuffer = Buffer.from(chartImage.split(",")[1], "base64");

        const chartWidth = 350;   // wider
        const chartHeight = 220;  // shorter height

        const centerX = (pageWidth - chartWidth) / 2;

        doc.image(chartBuffer, centerX, y, {
          width: chartWidth,
          height: chartHeight
        });
      ...
      const colW = Math.min(120, (pageWidth - 100) / headers.length);
      const tableY = y;
      ...
      headers.forEach((h, i) => {
        ...
        else if (key === "range_fore") txt = "RANGE FOREST";
        else if (key === "name_of_co") txt = "NAME OF COLONY";
      ...
      doc.fontSize(10)
        .fillColor("#555")
        .text(
          `Generated By RSAC UP • ${new Date().toLocaleDateString("en-GB")}`,
          50,
          rowY + 20
        );
